 ...
 */
import { evaluateMandate, MandateEngine, AuditDatabase, startDashboard, stopDashboard, AuthZenServer, } from '@clawdreyhepburn/ovid-me';
export const id = 'openclaw-ovid-me';
export const name = 'OVID-ME';
let engine = null;
let auditDb = null;
let dashboardRunning = false;
let authzenServer = null;
const sessionMandates = new Map();
function extractOvidFromTask(task) {
...
export default function register(api) {
    ...
    api.registerService({
        ...
        async start() {
            ...
            const policySource = {
                async getEffectivePolicy(principal) {
            ...
            const ovidConfig = {
                ...
                structuralFallback: config.structuralFallback ?? 'normalized',
            ...
            try {
                ...
                if (err?.code === 'OVID_SQLITE_UNAVAILABLE') {
                    logger.warn(`OVID-ME: SQLite audit store unavailable (native better-sqlite3 binding missing). ` +
    ...
    if (api.on) {
        ...
        api.on("after_tool_call", async (event, ctx) => {
            ...
            if (toolName === "sessions_spawn") {
                ...
                if (ovidMandate) {
                    ...
                    let childKey = '';
                    try {
                        const resultEnvelope = (typeof event.result === 'string' ? JSON.parse(event.result) : event.result);
                        ...
                        if (!childKey && Array.isArray(resultEnvelope?.content)) {
                            const textContent = resultEnvelope.content.find((c) => c.type === 'text');
                            if (textContent?.text) {
                    ...
                    if (childKey) {
                        ...
                        if (engine) {
                            try {
                                ...
                                const proofResult = await engine.verifySubset({ type: 'agent_mandate', rarFormat: 'cedar', policySet: ovidMandate.policySet }, parentIss);
                    ...
                    try {
                        ...
                        const rawChain = detail?.parentChain ??
                            detail?.parent_chain ??
                            payload.parent_chain ??
                            [];
                        const chainSubs = Array.isArray(rawChain)
                            ? rawChain.map((l) => (typeof l === 'string' ? l : (l?.sub ?? '')))
                            : [];
        ...
        api.on("before_tool_call", async (event, ctx) => {
            ...
            if (auditDb) {
                try {
                    auditDb.recordDecision(mandate.agentJti, action, `${resourceType}:${toolName}`, qualifiedDecision, matchedPolicy ? [matchedPolicy] : []);
                }
                catch (err) {
    ...
    api.registerTool({
        ...
        async execute(_toolCallId, params) {
            try {
                const request = { action: params.action, resource: params.resource };
